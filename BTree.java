...
public class BTree {
    ...
    private String search(Node node, int key) {
        int i = 1;
        while (i <= node.n && key > node.keys[i]) { // 找出最小下标i
            i = i + 1;
        }
        if (i <= node.n && key == node.keys[i]) { // 检查是否已经找到关键字
     ...
     * @param value
     */
    public void insert(BTree tree, int key, String value) {
        Node r = tree.root;
     ...
     * @param value
     */
    private void insertNotFull(Node node, int key, String value) {
        int i = node.n;
        if (node.isLeaf) { // 如果node是叶结点，直接将key和value插入node
            ...
            while (i >= 1 && key < node.keys[i]) { // 决定向node的那个子节点递归下降
                i = i - 1;
            }
            i = i + 1;
            if (node.childs[i].n == 2 * T - 1) { // 检查是否递归将至一个满子节点上
                splitChild(node, i); // 将该子节点分裂为两个非满孩子
                if (key > node.keys[i]) { // 决定向两个孩子中的哪一个下降是正确的
                    i = i + 1;
                }
            } else {
                insertNotFull(node.childs[i], key, value); // 递归地将key和value插入到合适的子树中
            }
        }
    }
